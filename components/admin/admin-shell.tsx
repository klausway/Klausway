  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-[1800px]">
      <aside className="hidden w-72 shrink-0 flex-col border-r border-black/10 bg-muted/60 p-6 lg:flex">
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  active
                    ? "bg-brand-500/15 text-brand-800 ring-1 ring-brand-400/20"
        ...
        <div className="mt-auto rounded-xl border border-black/10 bg-black/[0.03] p-4">
          <p className="truncate text-sm font-medium">{userName}</p>
          <p className="truncate text-xs text-muted-foreground">{userEmail}</p>
          <p className="mt-1 text-[10px] font-semibold uppercase tracking-wider text-brand-600/80">
            {roleLabel}
          </p>
          <button
            type="button"
        ...
        <header className="sticky top-0 z-20 border-b border-black/10 bg-background/95 px-4 py-3 backdrop-blur lg:hidden">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              type="button"
              onClick={onLogout}
              className="ml-auto inline-flex items-center gap-1.5 rounded-lg border border-black/10 px-2.5 py-1 text-xs"
            >
              <LogOut className="h-3.5 w-3.5" />
              Out
            </button>
